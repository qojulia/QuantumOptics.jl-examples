...
function convert2source(name)
    ...
    run(`jupyter-nbconvert --to=script --ExecutePreprocessor.kernel_name=julia-0.7 --ExecutePreprocessor.timeout=100 --output-dir=$juliadir $sourcepath`)
end

function convert2markdown(name)
    sourcepath = joinpath(sourcedir, name)
    run(`jupyter-nbconvert --to markdown --ExecutePreprocessor.kernel_name=julia-0.7 --ExecutePreprocessor.timeout=100 --output-dir=$markdowndir --template=markdown_template.tpl --execute $sourcepath `)
end
